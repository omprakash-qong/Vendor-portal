...
<div class="page-wrapper">
    <div class="qong-card">
        ...
        <a href="{{ route('login') }}" class="back-link">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 12H5M5 12l7 7M5 12l7-7"/></svg>
            Back to sign in
        </a>
        <h2 class="qong-heading">Reset Password</h2>
        <p class="qong-sub">Enter your vendor email and we'll send you a secure reset link.</p>
        @if (session('status'))
        ...
        <form method="POST" action="{{ route('password.email') }}" id="forgotForm">
            ...
            <div class="form-group" style="animation:fadeUp .5s .34s both">
                ...
                <div class="input-wrap"><svg class="input-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="3"/><path d="m2 7 10 7 10-7"/></svg>
                    <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email') }}" placeholder="vendor@company.com" required autofocus>
                </div>
                @error('email')<span class="error-message">{{ $message }}</span>@enderror
            </div>
            <button type="submit" class="btn-primary" id="forgotBtn" style="animation:fadeUp .5s .4s both">Send Reset Link</button>
        </form>
        <div class="qong-footer">QONG Systems &mdash; <span>Next-Engineered for Oil &amp; Gas</span></div>
        <script>document.getElementById('forgotForm').addEventListener('submit',function(){const b=document.getElementById('forgotBtn');b.classList.add('loading');b.textContent='Sending...';});<\/script>
    </div>
</div>
</x-guest-layout>