...
<div class="page-wrapper">
    <div class="qong-card">
        ...
        <h2 class="qong-heading">New Password</h2>
        <p class="qong-sub">Choose a strong password for your vendor account.</p>
        <form method="POST" action="{{ route('password.store') }}" id="resetForm">
            ...
            <div class="form-group" style="animation:fadeUp .5s .32s both">
                ...
                <div class="input-wrap"><svg class="input-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="3"/><path d="m2 7 10 7 10-7"/></svg>
                    <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $request->email) }}" placeholder="vendor@company.com" required autofocus autocomplete="username">
                </div>
                @error('email')<span class="error-message">{{ $message }}</span>@enderror
            </div>
            <div class="form-group" style="animation:fadeUp .5s .38s both">
                <label>New Password</label>
                <div class="input-wrap"><svg class="input-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="7.5" cy="15.5" r="5.5"/><path d="m21 2-9.6 9.6M15.5 7.5l3 3"/></svg>
                    <input type="password" name="password" class="form-control @error('password') is-invalid @enderror" placeholder="••••••••" required autocomplete="new-password">
                </div>
                @error('password')<span class="error-message">{{ $message }}</span>@enderror
            </div>
            <div class="form-group" style="animation:fadeUp .5s .44s both">
            ...
            <button type="submit" class="btn-primary" id="resetBtn" style="animation:fadeUp .5s .5s both">Reset Password</button>
        </form>
        <div class="qong-footer">QONG Systems &mdash; <span>Next-Engineered for Oil &amp; Gas</span></div>
        <script>document.getElementById('resetForm').addEventListener('submit',function(){const b=document.getElementById('resetBtn');b.classList.add('loading');b.textContent='Resetting...';});<\/script>
    </div>
</div>
</x-guest-layout>