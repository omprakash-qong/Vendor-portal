...
<style>    @import url('/css/qong-tokens.css');
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
    ...
    .verify-icon{width:58px;height:58px;border-radius:50%;background:rgba(200,0,255,.12);border:1.5px solid rgba(200,0,255,.4);display:flex;align-items:center;justify-content:center;margin:0 auto 1.2rem;box-shadow:0 0 24px rgba(200,0,255,.2)}
    .verify-icon svg{width:26px;height:26px;color:rgba(200,0,255,.9)}
    .btn-row{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:1.4rem}
</style>
...
<div class="page-wrapper">
    <div class="qong-card">
        ...
        <div class="verify-icon" style="animation:fadeUp .5s .22s both">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
                <polyline points="22,6 12,13 2,6"/>
            </svg>
        </div>
        <h2 class="qong-heading" style="text-align:center">Verify Your Email</h2>
        <p class="qong-sub" style="text-align:center;animation:fadeUp .5s .3s both">
            We've sent a verification link to your email. Click it to activate your vendor account.
        </p>
        @if (session('status') == 'verification-link-sent')
            <div class="alert-success">A new verification link has been sent to your email address.</div>
        @endif
        <div class="btn-row" style="animation:fadeUp .5s .36s both">
            <form method="POST" action="{{ route('verification.send') }}" style="flex:1" id="resendForm">
                @csrf
                <button type="submit" class="btn-primary" id="resendBtn">Resend Link</button>
            </form>
        ...
        <script>document.getElementById('resendForm').addEventListener('submit',function(){const b=document.getElementById('resendBtn');b.classList.add('loading');b.textContent='Sending...';});<\/script>
    </div>
</div>
</x-guest-layout>